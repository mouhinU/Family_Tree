...
| 数据库迁移 | Flyway（H2 + MySQL 双轨） | 10.x（V1–V11） |
...
- **世代辈分**：50 世辈分名（字辈）管理，节点显示世代号与辈分名，背景龙纹辈分水印并支持按世代圈点高亮。
...
- **香烛/烧纸**：为已故长辈上香烛或烧纸，详情弹框中展示祭奠统计面板与动效。
...
Flyway 在启动时自动执行迁移（V1~V11），`baseline-on-migrate: true`。
...
| `layout.js` | 方向无关布局算法、"只看健在"过滤树、"隐藏外嫁"过滤树 |
| `render.js` | D3 SVG 渲染（牌位式节点、连线、辈分水印、卷轴动画、缩放平移） |
| `export.js` | 独立 SVG 构建 + PDF 导出 |
| `modals.js` | 全部弹窗（节点增删改、详情/婚姻设置/祭奠、排次/辈分/颜色、家族管理、操作日志） |
| `context-menu.js` | 右键菜单（9 个操作分发） |
| `main.js` | 初始化入口、工具栏事件绑定 |
